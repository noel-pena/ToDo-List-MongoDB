...
app.post("/add", async (req, res) => {
  ...
  const input = await db.query("INSERT INTO items (title) values ($1)",
  [item]
  );
...
app.post("/edit", async (req, res) => {
  ...
  const pushUpdate = await db.query("UPDATE items SET title = ($1) WHERE id = ($2)",
  [updateTitle, id]
  );
...
app.post("/delete", async (req, res) => {
  ...
  const pushDelete = await db.query("DELETE FROM items WHERE id=($1)",
  [id]
  );
  console.log(id)
  res.redirect("/");
...
app.post("/edit-month", async (req, res) => {
  ...
  const updateTitle = req.body.updatedItemTitle;
  const id = req.body.updatedItemId;
  const pushUpdate = await db.query("UPDATE items_month SET title = ($1) WHERE id = ($2)",
  [updateTitle, id]
  );
  console.log(updateTitle)
  res.redirect("/month");
...
app.post("/edit-week", async (req, res) => {
  ...
  const updateTitle = req.body.updatedItemTitle;
  const id = req.body.updatedItemId;
  const pushUpdate = await db.query("UPDATE items_week SET title = ($1) WHERE id = ($2)",
  [updateTitle, id]
  );
  console.log(updateTitle)
  res.redirect("/week");
